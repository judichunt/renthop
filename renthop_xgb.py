# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 00:29:47 2017
...
input_folder = 'C:/Users/user/Downloads/renthop/'
...
def train_xgb(X, y, params):
    ...
    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.0005, random_state=RS)
...
def main():
    ...
    df['price_per_unit']=df['price']/(df['bathrooms']/2+df['bedrooms']+2)
    df['price2']=df['price']**2
    df['bathrooms']=df['bathrooms']**2 
    df['bedrooms']=df['bedrooms']**2
    
    	#df['temp']=np.sum(np.divide(df['bathrooms'],2),df['bedrooms'])
    	#df['temp']=np.sum(df['bathrooms'],df['bedrooms'])
    	#df['price_per_unit']=np.divide(df['price'],df['temp'])
